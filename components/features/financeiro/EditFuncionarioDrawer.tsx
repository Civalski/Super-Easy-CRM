...
interface EditFuncionarioDrawerProps {
  open: boolean
  onClose: () => void
...
export default function EditFuncionarioDrawer({ open, onClose, funcionarioId, onSaved }: EditFuncionarioDrawerProps) {
  ...
  useEffect(() => {
    if (!open || !funcionarioId) {
      setForm({ nome: '', cargo: '', departamento: '', email: '', telefone: '', documento: '', observacoes: '' })
      return
    }
    setLoading(true)
    fetch(`/api/financeiro/funcionarios/${funcionarioId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data?.id) {
          setForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.nome.trim() || !funcionarioId) return
    setSaving(true)
    try {
      const res = await fetch(`/api/financeiro/funcionarios/${funcionarioId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(form),
      })
      const data = await res.json().catch(() => null)
      if (!res.ok) throw new Error(data?.error || 'Erro ao atualizar funcionario')
      onSaved()
      onClose()
    } catch (err) {
      console.error(err)
      ...
      <div className="h-full overflow-y-auto">
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Editar Funcionario</h2>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Altere os dados do funcionario</p>
            </div>
          </div>
          <button
            type="button"
