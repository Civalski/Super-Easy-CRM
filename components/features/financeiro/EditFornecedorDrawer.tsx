...
interface EditFornecedorDrawerProps {
  open: boolean
  onClose: () => void
...
export default function EditFornecedorDrawer({ open, onClose, fornecedorId, onSaved }: EditFornecedorDrawerProps) {
  ...
  useEffect(() => {
    if (!open || !fornecedorId) {
      setForm({ nome: '', empresa: '', email: '', telefone: '', documento: '', endereco: '', cidade: '', estado: '', cep: '', observacoes: '' })
      return
    }
    setLoading(true)
    fetch(`/api/financeiro/fornecedores/${fornecedorId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data?.id) {
          setForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.nome.trim() || !fornecedorId) return
    setSaving(true)
    try {
      const res = await fetch(`/api/financeiro/fornecedores/${fornecedorId}`, {
        method: 'PATCH',
      ...
      <div className="h-full overflow-y-auto">
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Editar Fornecedor</h2>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Altere os dados do fornecedor</p>
            </div>
          </div>
          <button
            type="button"
