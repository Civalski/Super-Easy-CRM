...
export function AtividadesRecentes({ refreshTrigger, onRefreshRequest }: AtividadesRecentesProps) {
    ...
    const handleActivityUpdate = () => {
        // Se houver uma função de atualização global, chama ela
        if (onRefreshRequest) {
            onRefreshRequest()
        } else {
            // Caso contrário, atualiza apenas a lista local
            fetchActivities()
        }
    }
    ...
    const getActionText = (type: string) => {
        switch (type) {
            case 'tarefa': return 'Nova tarefa criada'
            case 'oportunidade': return 'Nova oportunidade'
            case 'cliente': return 'Novo cliente cadastrado'
