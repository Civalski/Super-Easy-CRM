...
export async function GET(
  ...
  return withAuth(request, async (userId) => {
    try {
      ...
      const [pedido, user, pdfConfig] = await Promise.all([
      ...
      const companyName = pdfConfig?.nomeEmpresa || 'PEDIDO'
      ...
      if (pedido.dataEntrega) {
        const entregaLabel = `Entrega prevista: ${dateBr(pedido.dataEntrega)}`
        const entregaLW = font.widthOfTextAtSize(entregaLabel, FS.small)
        page.drawText(entregaLabel, {
          x: docRightX - entregaLW, y: PAGE.height - 57,
          size: FS.small, font, color: C.textLight,
        })
      }
      ...
      page.drawText('TÍTULO DO PEDIDO', {
        x: PAGE.marginX + 10, y: y - 11,
        size: FS.label, font: fontBold, color: C.textMuted,
      })
      page.drawText(truncateToWidth(pedido.oportunidade.titulo, fontBold, 11, CONTENT_WIDTH - 24), {
        x: PAGE.marginX + 10, y: y - 26,
        size: 11, font: fontBold, color: C.textDark,
      })
      y -= TITLE_H + 10

      const cliente = pedido.oportunidade.cliente
      ...
      const summaryCards = [
        { label: 'STATUS ENTREGA', value: STATUS_ENTREGA_LABEL[pedido.statusEntrega] ?? pedido.statusEntrega },
        { label: 'FORMA PGTO.',    value: pedido.formaPagamento || pedido.oportunidade.formaPagamento || '-' },
        { label: 'PAGAMENTO',      value: pedido.pagamentoConfirmado ? 'Confirmado' : 'Pendente' },
        { label: 'DATA ENTREGA',   value: pedido.dataEntrega ? dateBr(pedido.dataEntrega) : '-' },
      ]
      ...
      if (pedido.oportunidade.descricao) {
        const descLines = wrapText(pedido.oportunidade.descricao, font, FS.text, CONTENT_WIDTH - 16)
      ...
      drawTotalRow('Total bruto:',  currency(pedido.totalBruto),    false, false, 14)
      drawTotalRow('(-) Desconto:', currency(pedido.totalDesconto), false, false, 30)
      page.drawLine({
        start: { x: TBOX_X + 8, y: y - 38 }, end: { x: TBOX_X + TBOX_W - 8, y: y - 38 },
        thickness: 0.5, color: C.border,
      })
      drawTotalRow('Total líquido:', currency(pedido.totalLiquido), true, true, 56)

      y -= TBOX_H + 14

      if (pedido.observacoes) {
        const noteLines = wrapText(pedido.observacoes, font, FS.text, CONTENT_WIDTH - 16)
        ensure(20 + noteLines.length * 13)
        page.drawText('Observações do pedido:', { x: PAGE.marginX, y, size: FS.label, font: fontBold, color: C.textMuted })
        y -= 13
      ...
      for (let i = 0; i < totalPages; i++) {
        ...
        const leftFooter = pdfConfig?.rodape
          ...
          : truncateToWidth(`Emissão: ${emissaoStr}`, font, FS.small, footerMaxW)
      ...
      return new NextResponse(pdfBlob, {
        ...
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': `attachment; filename="Pedido ${docNum} - ${emissaoDateForFile}.pdf"; filename*=UTF-8''${encodedFileName}`,
          'Cache-Control': cacheControl,
          ETag: etag,
        },
      })
