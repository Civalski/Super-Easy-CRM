...
import { Filter, Send } from '@/lib/icons'
import {
  CreateContratoModal,
  ContratosList,
  EditContratoDrawer,
...
export default function PropostasPage() {
  ...
  const handleDownloadPdf = useCallback(async (c: Contrato) => {
    try {
      ...
      a.href = url
      a.download = getDownloadFileNameFromHeader(res.headers.get('Content-Disposition')) || 'Proposta.pdf'
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        {!minimal && (
          <div className="flex items-center gap-3">
            <div className="rounded-xl bg-linear-to-br from-sky-500 to-indigo-600 p-2.5 shadow-lg shadow-sky-500/25">
              <Send className="h-6 w-6 text-white" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Proposta</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Escopo comercial, condições e PDF — sem cláusulas de contrato
              </p>
            </div>
          </div>
        )}
